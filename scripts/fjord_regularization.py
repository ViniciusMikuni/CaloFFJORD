...
def trace_jacobian_hutchinson(
  ...
  def augmented_ode_fn(time, state_log_det_jac, **kwargs):
    ...
    def estimate_trace_norm(random_sample):
      ...
      trace_val = random_sample * jvp
      norm_val = jvp**2  
      return tf.stack([trace_val, norm_val])
      
    # TODO(dkochkov) switch to vectorized_map once more features are supported.
    combined_results = tf.map_fn(estimate_trace_norm, random_samples)
    trace_results, norm_results = combined_results[:,0], combined_results[:,1]
    trace_estimates = tf.reduce_mean(trace_results, axis=0)
    norm_estimates = tf.reduce_mean(norm_results, axis=0)
    kinetic_estimates = state_time_derivative**2
    
    return state_time_derivative, trace_estimates, kinetic_estimates, norm_estimates
  ...
  * solution_times: 1D floating Tensor of solution times.
  ...
  def _augmented_forward(self, x, **condition_kwargs):
    ...
    if condition_kwargs:
      ...
      y, fldj, kinetic_penalty, jacobian_penalty = self._solve_ode(augmented_ode_fn, augmented_x)
    return y, {'ildj': -fldj, 'fldj': fldj, 'kinetic_penalty': kinetic_penalty, 'jacobian_penalty': jacobian_penalty}
  ...
  def _augmented_inverse(self, y, **condition_kwargs):
    ...
    augmented_y = (y, tf.zeros_like(y), tf.zeros_like(y), tf.zeros_like(y))
    if condition_kwargs:
  ...
  def _solve_ode_timesteps(self, ode_fn, state, step_size, **kwargs):
    integration_result = self._ode_solve_fn(
        ...
        initial_time=self._initial_time,
        initial_state=state,
        solution_times=tf.range(step_size, self._final_time + step_size, delta=step_size),
        **kwargs)
    final_state = tf.nest.map_structure(
        lambda x: x, integration_result.states)
    print(integration_result.diagnostics.num_ode_fn_evaluations)
    return final_state

  def _augmented_forward_timesteps(self, x, step_size, **condition_kwargs):
    augmented_ode_fn = self._trace_augmentation_fn(
        self._state_time_derivative_fn, prefer_static.shape(x[-1]), x.dtype)
    augmented_x = (x[-1], tf.zeros_like(x[-1]), tf.zeros_like(x[-1]), tf.zeros_like(x[-1]))
    if condition_kwargs:
      y, fldj, kinetic_penalty, jacobian_penalty = self._solve_ode_timesteps(augmented_ode_fn, augmented_x, step_size,
                                constants=condition_kwargs)
    else:
      y, fldj, kinetic_penalty, jacobian_penalty = self._solve_ode_timesteps(augmented_ode_fn, augmented_x, step_size)
    return y, {'ildj': -fldj, 'fldj': fldj, 'kinetic_penalty': kinetic_penalty, 'jacobian_penalty': jacobian_penalty}
  ...
  def _forward(self, x, **kwargs):
    y, _ = self._augmented_forward(x, **kwargs)
    return y
  ...
  def _forward_timesteps(self, x, step_size, **condition_kwargs):
    y, _ = self._augmented_forward_timesteps(x, step_size, **condition_kwargs)
    return y
